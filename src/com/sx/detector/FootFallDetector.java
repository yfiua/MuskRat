...
public class FootFallDetector implements SensorEventListener {
	...
	private static final int NUMBER_OF_FOOT_FALLS = 9;
	...
	// get current cadence in steps/min
	public synchronized int getCurrentCadence() {
		...
		try {
			...
			float threshold = (float) Math.abs(g * 0.618);
			...
			Log.d(TAG, "Not enough sensor events");
			return 0;
		}
	}

	// Calculate cadence by timestamp of last foot falls, return the average
	private int calculateCadenceByFootFallTimestamp(long[] footFallTimestamps) {
		...
		long average = sum / (NUMBER_OF_FOOT_FALLS - 1);
		return (int) (60 * SECOND_TO_NANOSECOND / average);

	}

	// find vertical axis (always max) 
